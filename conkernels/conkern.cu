...
__global__ void clock_block(clock_t *d_o, clock_t clock_count) {
  ...
  printf("in clock() , clock_count is %ld\n",(long int) clock_count);
  while (clock_offset < clock_count) {
    ...
    clock_offset = (clock_t)(end_clock - start_clock);
    //printf("in  clock(), %ld-%ld=%ld\n",(long int) end_clock,(long int)start_clock, (long int) clock_offset);
  }
  //printf("clock_offset > clock_count now\n");
  d_o[0] = clock_offset;
  printf("in  clock(), d_o[0]=%ld\n", (long int)clock_offset);
}
...
__global__ void sum(clock_t *d_clocks, int N) {
  ...
  for (int i = threadIdx.x; i < N; i += blockDim.x) {
    my_sum = d_clocks[i];
    printf(" threadidx.x=%d,i=%d,mysum=%ld,blockDim.x=%d,N=%d,d_clocks[i]=%ld\n",threadIdx.x,i,(long int) my_sum,blockDim.x,N,(long int)d_clocks[i]);
   
  }

  s_clocks[threadIdx.x] = my_sum;
  // printf(" threadidx.x=%d,s_slock=%ld\n",threadIdx.x,(long int)s_clocks[threadIdx.x]);
  cg::sync(cta);
  
  for (int i = 16; i > 0; i /= 2) {
    
    if (threadIdx.x < i) {
      //printf("before addition %ld,%ld,threadidx.x=%d,i=%d\n",(long int)s_clocks[threadIdx.x],(long int)s_clocks[threadIdx.x + i],threadIdx.x,i);
      s_clocks[threadIdx.x] += s_clocks[threadIdx.x + i];
     // printf("after addition %ld,%ld,threadidx.x=%d,i=%d\n",(long int)s_clocks[threadIdx.x],(long int)s_clocks[threadIdx.x + i],threadIdx.x,i);
    }else {
        //printf("else-case %ld,%ld,threadidx.x=%d,i=%d\n",(long int)s_clocks[threadIdx.x],(long int)s_clocks[threadIdx.x + i],threadIdx.x,i);
    }
    
   
    cg::sync(cta);
  }
  //printf("final s_clocks[0]=%ld\n",s_clocks[0]);
...
int main(int argc, char **argv) {
...
#endif
  printf("deviceprop clockrate is =%d\n",deviceProp.clockRate);
  ...
  printf("total clock counters  while running %d kernels  concurrently =%ld\n",nkernels,(long int) a[0]);
  printf("total clock counters while running  %d kernels serially =%ld\n",nkernels,(long int) total_clocks);
